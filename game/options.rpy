...
define build.name = "DokiDokiModLauncher"
...
define config.save_directory = "Doki_Doki_Mod_Launcher"
...
init python:
    ...
    build.package(build.directory_name + "Mod", 'zip',build.name,description='Doki Doki Mod Launcher')

    # archives to create
    build.archive("DDML",build.name)
    build.archive("DDLC",build.name)
    #build.archive("submods",build.name)

    # folder / files to put in archives
    build.classify("game/mod_assets/**", None)
    build.classify('game/ddml/**.rpyc',"DDML")
    build.classify('game/options.rpyc',None)
    build.classify('game/modlist.rpyc', None)
    build.classify('game/characters/**.chr', None)
    build.classify('game/ddlc/**.rpyc',"DDLC")
    build.classify('game/**.rpyc',"DDLC")
    build.classify('game/**.chr',"DDLC")
    build.classify('game/**.txt',"DDLC")
