    ...
    def game_installation(self, filePath, modFolder, copy=False):
        ...
        for contents in os.listdir(game_dir):
            ...
            if os.path.isdir(os.path.join(game_dir, contents)):
                ...
                shutil.copytree(
                    os.path.join(game_dir, contents), os.path.join(modFolder, contents)
                )
        ...
        if len(os.listdir(td)) > 1 or "game" in os.listdir(td):
            ...
            for x in os.listdir(td):
                ...
                if x != "ImproperMod":
                    ...
                    if (
                        not x.endswith(self.renpy_executables)
                        and not x in self.renpy_base_contents
                        ...
                        shutil.move(
                            os.path.join(td, x),
                            os.path.join(td, "ImproperMod", "game", x),
                        )

                    else:

                        if x == "game" and os.path.exists(
                            os.path.join(td, "ImproperMod", "game")
                        ):

                            for y in os.listdir(os.path.join(td, "game")):
                                if os.path.isdir(os.path.join(td, "game", y)):
                                    shutil.move(
                                        os.path.join(td, "game", y),
                            ...
                            shutil.move(
                                os.path.join(td, x), os.path.join(td, "ImproperMod", x)
                            )
            ...
            if contents == "game":
                ...
                for x in os.listdir(os.path.join(mod_dir, contents)):
                    ...
                    if os.path.isdir(os.path.join(mod_dir, contents, x)):
                        ...
                        shutil.copytree(
                            os.path.join(mod_dir, contents, x),
                ...
                shutil.copytree(
                    os.path.join(mod_dir, contents), os.path.join(modFolder, contents)
                )
